	  ...
	  <script type="text/javascript">
		  $(function(){
			  $("#next").click(function(){
				  ...
				  page++;
				  location.href = "/user/approval?page="+page;
			  })
			  ...
			  $("#first").click(function () {
				  location.href = "/user/approval?page=1";
			  })

			  $("#last").click(function(){
				  var pages = parseInt($("#pages").html());
				  location.href = "/user/approval?page="+pages;
			  })
		  })
	  </script>
  </head>
		...
		<table class="table" cellspacing="0">
			...
			<c:forEach items="${resultList}" var="resultList">
				<tr>
					...
					<td>
						<a href="/update/repeal?bookId=${resultList.bookId}&borrowId=${resultList.borrowId}">撤销</a>
					</td>
<%--					<td>--%>
<%--						<c:if test="${resultList.state == 已拒绝}">--%>
<%--							<font color="red">${resultList.state}</font>--%>
<%--						</c:if>--%>
<%--						<c:if test="${resultList.state == 已归还}">--%>
<%--							<font color="green">${resultList.state}</font>--%>
<%--						</c:if>--%>
<%--					</td>--%>
				</tr>
			</c:forEach>
